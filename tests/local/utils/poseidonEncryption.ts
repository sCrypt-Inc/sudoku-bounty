...
const { mulPointEscalar } = require('./babyjub.js')
const { pruneBuffer } = require('./eddsa.js')
...
const genEcdhSharedKey = (
    ...
    return mulPointEscalar(pubKey, formatPrivKeyForBabyJub(privKey))
}

const formatSharedKey = (
    QsxArray: BigInt[]
): EcdhSharedKey => {
    let ks0 = BigInt("0x" + QsxArray[0].toString(16).substring(0, 16) + QsxArray[1].toString(16).substring(0, 16));
    let ks1 = BigInt("0x" + QsxArray[2].toString(16).substring(0, 16) + QsxArray[3].toString(16).substring(0, 16));
    return [ks0, ks1];
 }
