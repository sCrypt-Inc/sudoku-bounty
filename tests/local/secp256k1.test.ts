...
describe("Secp256k1AddUnequal", function () {
    ...
    let test_cases: Array<[bigint, bigint, bigint, bigint, bigint, bigint]> = [];

    // 4 randomly chosen private keys
    let privkeys: Array<bigint> = [88549154299169935420064281163296845505587953610183896504176354567359434168161n,
                                   37706893564732085918706190942542566344879680306879183356840008504374628845468n,
                                   90388020393783788847120091912026443124559466591761394939671630294477859800601n,
                                   110977009687373213104962226057480551605828725303063265716157300460694423838923n];
    let pubkeys: Array<Point> = [];
    ...
    for (let idx = 0; idx < 4; idx++) {
        for (let idx2 = idx + 1; idx2 < 4; idx2++) {
            let sum: Point = pubkeys[idx].add(pubkeys[idx2]);
    ...
    let test_secp256k1_add_instance = function (test_case: [bigint, bigint, bigint, bigint, bigint, bigint]) {
        ...
        let pub0x_array: bigint[] = bigint_to_array(64, 4, pub0x);
        let pub0y_array: bigint[] = bigint_to_array(64, 4, pub0y);
        let pub1x_array: bigint[] = bigint_to_array(64, 4, pub1x);
        let pub1y_array: bigint[] = bigint_to_array(64, 4, pub1y);
        let sumx_array: bigint[] = bigint_to_array(64, 4, sumx);
        let sumy_array: bigint[] = bigint_to_array(64, 4, sumy);
...
describe("Secp256k1Double", function () {
    ...
    for (let idx = 0; idx < 4; idx++) {
        let double: Point = pubkeys[idx].add(pubkeys[idx]);
        test_cases.push([pubkeys[idx].x, pubkeys[idx].y,
                         double.x, double.y]);
    }

    let test_secp256k1_double_instance = function (test_case: [bigint, bigint, bigint, bigint]) {
        let pubx = test_case[0];
        let puby = test_case[1];
        let doublex = test_case[2];
        let doubley = test_case[3];

        let pubx_array: bigint[] = bigint_to_array(64, 4, pubx);
        let puby_array: bigint[] = bigint_to_array(64, 4, puby);
        let doublex_array: bigint[] = bigint_to_array(64, 4, doublex);
        let doubley_array: bigint[] = bigint_to_array(64, 4, doubley);
...
describe("Secp256k1ScalarMult", function () {
    ...
    let test_cases: Array<[bigint, bigint, bigint, bigint, bigint]> = [];
    ...
    let privkeys: Array<bigint> = [88549154299169935420064281163296845505587953610183896504176354567359434168161n,
                                   ...
                                   90388020393783788847120091912026443124559466591761394939671630294477859800601n,
                                   110977009687373213104962226057480551605828725303063265716157300460694423838923n];
    let scalars: Array<bigint> = [49162213912846590284921918007854216316029616890568331838502149777137252900653n,
    ...
    for (let idx = 0; idx < 4; idx++) {
        let scalar: bigint = scalars[idx];
        let scalar_point: Point = pubkeys[idx].multiply(scalar);
        test_cases.push([scalar, pubkeys[idx].x, pubkeys[idx].y,
                         scalar_point.x, scalar_point.y]);
    }

    let test_secp256k1_scalar_instance = function (test_case: [bigint, bigint, bigint, bigint, bigint]) {
        ...
        let scalar_array: bigint[] = bigint_to_array(64, 4, scalar);
        let pubx_array: bigint[] = bigint_to_array(64, 4, pubx);
        let puby_array: bigint[] = bigint_to_array(64, 4, puby);
        let scalarx_array: bigint[] = bigint_to_array(64, 4, scalarx);
        let scalary_array: bigint[] = bigint_to_array(64, 4, scalary);
...
describe("Secp256k1PointOnCurve", function () {
    ...
    // x, y, on/off
    let test_cases: Array<[bigint, bigint, boolean]> = [];
    ...
    let test_secp256k1_poc_instance = function (test_case: [bigint, bigint, boolean]) {
        let x = test_case[0];
        let y = test_case[1];
        let on_curve = test_case[2];

        let x_array: bigint[] = bigint_to_array(64, 4, x);
        let y_array: bigint[] = bigint_to_array(64, 4, y);
